...
namespace
{
    ...
    constexpr const char *MEASUREMENTS_LOG_HEADER = "timestamp,measurement_type,value,unit,session_id,source,global_id";
...
bool Logger::prepare_log_directories_locked()
{
    ...
    if (!SD.exists(MEASUREMENTS_LOG_DIR_PATH) && !SD.mkdir(MEASUREMENTS_LOG_DIR_PATH))
    {
        Serial.printf("[Logger] Failed to create dir: %s\n", MEASUREMENTS_LOG_DIR_PATH);
        return false;
    }
...
{
    // Use date-based naming: measurements_YYYY-MM-DD.csv (no rotation by sequence)
    time_log_file_index++;
    time_log_entries_in_file = 0;
    
    // Format the path with date (using the current session timestamp)
...
bool Logger::ensure_log_file_locked(bool for_time_log)
{
    ...
    const char *header = for_time_log ? MEASUREMENTS_LOG_HEADER : EVENT_LOG_HEADER;
...
void Logger::log_time_sample(time_t timestamp, float temperature, unsigned long time_id)
{
    ...
    Serial.printf("[Measurement][%s] %s,temperature,%.1f,°C,%s,device_001,%lu\n", target_path, time_str, temperature, session_tag, time_id);
    ...
    if (log_file)
    {
        // New standardized format: timestamp,measurement_type,value,unit,session_id,source,global_id
        log_file.printf("%s,temperature,%.1f,°C,%s,device_001,%lu\n", time_str, temperature, session_tag, time_id);
        log_file.flush();
        log_file.close();
